...
export default function TestPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      setQuizState((prev) => ({
        ...prev,
        timeRemaining: testData.time_limit_minutes
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl border border-slate-200 p-10 text-center max-w-md">
          <h2 className="text-xl font-bold text-slate-900 mb-2">No Test Available</h2>
          <p className="text-slate-500 mb-6">
            There is no test available for this lesson yet.
          </p>
          <Link href={`/programs/${slug}/learn/${courseSlug}`}>
    ...
    return (
      <div className="min-h-screen bg-slate-50 py-8 px-4">
        <div className="max-w-2xl mx-auto">
          <Link
            href={`/programs/${slug}/learn/${courseSlug}`}
          ...
          <div className="flex items-center gap-1">
            {questions.map((_, i) => (
              <button
                key={i}
                onClick={() =>
                  setQuizState((prev) => ({ ...prev, currentQuestion: i }))
                }
                className={cn(
                  'h-2 w-2 rounded-full transition-all',
                  i === quizState.currentQuestion
                    ? 'bg-blue-600 w-4'
                    : quizState.answers[questions[i].id]
                      ? 'bg-blue-300'
                      : 'bg-slate-200'
                )}
              />
